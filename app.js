...
async function addItem(listId) {
    ...
    // Si es lista con precios, intentar obtener el precio (pero es opcional)
    if (list.type === 'price') {
        ...
        // Si no hay precio, price queda como null (se puede agregar después)
    }
            ...
            <div class="card">
                <div class="list-header">
                    <div>
                        ...
                        ${list.type === 'price' ? `<small style="display: block; margin-top: 3px;">Total: $${totalPrice.toFixed(0)} (${itemsWithPrice}/${total} con precio)</small>` : ''}
                    </div>
...
function renderItem(listId, item, listType) {
    ...
    if (listType === 'price') {
        if (item.price !== null && item.price !== undefined) {
            priceDisplay = `<span class="item-price">$${item.price.toFixed(0)}</span>`;
        } else {
            priceDisplay = `<span class="item-price-empty" onclick="startEdit('${listId}', '${item.id}')">Sin precio</span>`;
        }
    }
